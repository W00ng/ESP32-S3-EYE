...
#define I2S_CHN I2S_NUM_0   //i2s通道
    ...
    do {
        ...
        if (offset >= 0) {
            ...
            if (sample_rate != frame_info.samprate) {
                ...
                i2s_set_clk(I2S_CHN, sample_rate, bits_cfg, channel);
            }

            /* Write decoded data to audio decoder */
            size_t i2s_bytes_written = 0;
            size_t output_size = frame_info.outputSamps * frame_info.nChans;
            i2s_write(I2S_CHN, output, output_size, &i2s_bytes_written, portMAX_DELAY);
        } else {
...
static void audio_task(void *pvParam)
{
    ...
    bsp_i2s_init(I2S_CHN, 44100, false, false);
